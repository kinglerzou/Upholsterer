$(document).ready(function () {
    ...
    var vocation = ['软件','硬件','金融','制造','航天','互联网'];
    ...
    var companynature = ['国企', '外资非欧美', '私企', '外资欧美', '其他性质'];
    ...
    initSelete($("#CompanyNature"), companynature);
    initSelete($("#State"), stas);
    initSelete($("#Position"), pos);
    initSelete($("#Vocation"),vocation);

    //初始化<select><option>函数
 ...
 $(".selecttitle a").each(function (n) {
    ...
    $(".say .btn").click(function(e) {
        ...
        if (enable) {
            $.post("/User/SendPersonalState", { "content": but.val() }, function (data) {
                if (data == 0) {
                    $("#textempty").show().html("你更新太快了,还不到1分钟呢");
    ...
    function processJson(data) {
        ...
        $("#sCompany").html(data.CompanyNature);
