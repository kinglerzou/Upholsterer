$(function () {
        ...
        $(this).click(function () {
            ...
            switch (n) {
                case 0:
                    ...
                    text = city != "不限" ? "(所在地)" + prov + "-" +city : "(所在地)" + prov;
                    $(".selectedconditon .spanbox>span>span").eq(0).attr("title", text);
                    //lenth 最大为11 否则就溢出了 如果要大于11 要处理
                    if (text.length > 11) {
                        text = text.substring(10, length) + "...";
                    }
                    break;
                case 1:
    ...
    function search() {
        ...
        //所在地省份
        var place = $span.eq(0).html();
        var ptitle = $span.eq(0).attr("title");
        if (place.indexOf('...') > 0) {
            $("#ProvBelong").val((ptitle.split(")")[1]).split("-")[0]);
        } else {
            $("#ProvBelong").val((place.split(")")[1]).split("-")[0]);
        };
        //所在地城市
        if (place.indexOf("...") > 0) {
            $("#CityBelong").val((ptitle.split(")")[1]).split("-")[1]);
        ...
        //民族
        $("#People").val($span.eq(8).html());
        //状态
        $("#State").val($span.eq(9).html());
  ...
  '畲族', '僳僳族', '仡佬族', '东乡族', '水族', '佤族', '羌族', '仫佬族'];
    ...
    // 产生搜索return  partialview(simpleUsers)    
    $('#Form1').ajaxForm({
